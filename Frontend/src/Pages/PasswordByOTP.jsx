...
function PasswordOTP() {
  let { canSendOTP, setCanSendOTP, authToken, user } = useAllContexts();
  ...
  let resetPass = async (otpSys, otpSent) => {
    try {
      ...
      let response = await fetch(
        `${baseURL}/user/api/updatepassbyotp/${user?.email}/${otpSys}/${otpSent}`,
        {
          method: "PUT",
          headers: {
      ...
      if (
        resetData.success &&
        resetData.message === "Password updated successfully"
      ) {
        navigate("/");
      }
